...
var manifestHelloDefaultNodeJS = `
package:
  name: helloworld
  actions:
    helloNodejs:
...
func TestUnmarshalForHelloNodeJS(t *testing.T) {
    ...
    // else, it sets the values of
    err := NewYAMLParser().Unmarshal([]byte(manifestHelloNodeJS), &m)
    if err == nil {
